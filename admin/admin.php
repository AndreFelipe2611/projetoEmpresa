...
<script>
        ...
        fetch(`admin-handler.php?editar_categoria=${id}`, {
          ...
          .then(data => {
              if (data.status === 'success') {
                  const section = document.querySelector(`.section h2 span a[onclick*='${id}']`).closest('.section');
                  section.querySelector('h2').childNodes[0].textContent = nome;
                  section.style.borderLeft = `10px solid ${cor}`;
    ...
    function salvarItem() {
        ...
        fetch(`admin-handler.php?editar_item=${id}`, {
          ...
          .then(data => {
              if (data.status === 'success') {
                  const li = document.querySelector(`.btn-editar[onclick*='${id}']`).closest('li');
                  const nomeEl = li.querySelector('a') || li.querySelector('span');

                  if (link) {
                      if (!nomeEl || nomeEl.tagName !== 'A') {
                          const a = document.createElement('a');
                          a.href = link;
                          a.target = '_blank';
                          a.textContent = nome;
                          if (nomeEl) nomeEl.replaceWith(a);
                          else li.prepend(a);
                      } else {
                          nomeEl.href = link;
